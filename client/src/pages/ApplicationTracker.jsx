                    ...
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                            ...
                            {loading ? (
                                ...
                                const step3Color = ['validated'].includes(app.status) ? 'bg-green-600' : 'bg-slate-200 dark:bg-slate-700';
                                    ...
                                    <div key={app._id} className={`grid grid-cols-12 gap-4 px-6 py-6 border-b border-slate-50 items-center hover:bg-${info.colorClass}-50/30 transition-colors group`}>
                                        ...
                                        <div className="col-span-3">
                                            ...
                                            <div className="flex items-center w-full max-w-[200px]">
                                                ...
                                                <div className={`flex-1 h-0.5 ${step2Color !== 'bg-slate-200 dark:bg-slate-700' ? `bg-${info.colorClass}-600` : 'bg-slate-200 dark:bg-slate-700'}`}></div>
                                                ...
                                                {/* Step 3: Validated */}
                                                <div className="relative flex items-center justify-center">
                                                    <div className={`w-3 h-3 rounded-full ${step3Color} ${['accepted', 'rejected'].includes(app.status) ? `ring-4 ring-green-50 border-2 border-green-600 bg-white dark:bg-slate-800 w-4 h-4` : ''}`}></div>
                                                </div>
                                            </div>
                                            <div className="flex justify-between w-full max-w-[200px] mt-1">
                                                ...
                                                <span className={`text-[10px] font-bold ${['accepted', 'rejected'].includes(app.status) ? `text-${info.colorClass}-600` : 'text-slate-400 dark:text-slate-500'}`}>
                                                    {app.status === 'rejected' ? 'Refused' : 'Accepted'}
                                                </span>
                                                <span className={`text-[10px] font-bold ${app.status === 'validated' ? 'text-green-600' : 'text-slate-400 dark:text-slate-500'}`}>Validated</span>
                                            </div>
                                        </div>
                                        <div className="col-span-2">
        ...
        <StudentDeleteAlert
            ...
            candidate={deleteTarget ? {
                ...
                statusColor: deleteTarget.status === 'applied' ? 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'
                    ...
                    : deleteTarget.status === 'rejected' ? 'bg-red-100 text-red-700'
