                        ...
                        <div className="lg:col-span-8 xl:col-span-8 flex flex-col gap-8">
                            ...
                            <section>
                                <div className="flex items-center justify-between mb-4">
                                    <h2 className="font-display text-xl font-bold text-text-main dark:text-gray-100">{t('Active Applications')}</h2>
                                    <button onClick={() => navigate('/ApplicationTracker')} className="text-sm font-medium text-primary hover:underline">{t('viewAll')} ({applications.length})</button>
                                </div>
                                ...
                                {applications.length === 0 ? (
                                    <div className="bg-surface-light dark:bg-surface-dark rounded-xl shadow-lift p-8 border border-border-color dark:border-slate-700/50 flex flex-col items-center text-center gap-3">
                                        ...
                                        <p className="text-text-muted dark:text-gray-400 font-medium">{t('noApplications')}</p>
                                    </div>
                                ) : (() => {
                                        ...
                                        <div className="bg-surface-light dark:bg-surface-dark rounded-xl shadow-lift p-6 border border-border-color dark:border-slate-700/50">
                                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-text-muted dark:text-gray-400">{app.offerId?.companyId?.companyName || 'Company'} • {t('appliedOn')} {dateApplied}</p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="py-4">
                                                ...
                                                <div className="flex justify-between w-full mt-3">
                                                    <span className={`text-xs font-bold ${app.status === 'admin_rejected' ? 'text-green-600' : `text-${info.colorClass}-600`} text-center w-10`}>{t('Applied')}</span>
                                                    <span className={`text-xs font-bold text-center flex-1 ${['accepted', 'rejected', 'admin_rejected'].includes(app.status) ? (app.status === 'admin_rejected' ? 'text-green-600' : `text-${info.colorClass}-600`) : 'text-slate-400 dark:text-slate-500'}`}>
                                                        {app.status === 'rejected' ? t('Refused') : t('Accepted')}
                                                    </span>
                                                    <span className={`text-xs font-bold text-center w-10 ${app.status === 'validated' ? 'text-green-600' : (app.status === 'admin_rejected' ? 'text-red-600' : 'text-slate-400 dark:text-slate-500')}`}>
                                                        {app.status === 'admin_rejected' ? t('Refused') : t('Validated')}
                                                    </span>
                                                </div>
                                            </div>
                            ...
                            <section>
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-2">
                                        <h2 className="font-display text-xl font-bold text-text-main dark:text-gray-100">{t('suggestedForYou')}</h2>
                                        <span className="bg-secondary/10 text-secondary text-xs font-bold px-2 py-0.5 rounded-full">{t('aiMatched')}</span>
                                    </div>
